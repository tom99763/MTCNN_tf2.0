...
def _process_image_withoutcoder(filename):
    ...
    image = cv2.imread(f'./{dir}/'+filename.replace('.jpg.jpg','.jpg'))
...
def get_dataset(dir):
    ...
    for line in imagelist.readlines():
        ...
        if 'landmark' in info[0]:
            bbox['xmin'] = float(info[2])
            bbox['ymin'] = float(info[3])
            bbox['xmax'] = float(info[4])
            bbox['ymax'] = float(info[5])
            bbox['xlefteye'] = float(info[6])
            bbox['ylefteye'] = float(info[7])
            bbox['xrighteye'] = float(info[8])
            bbox['yrighteye'] = float(info[9])
            bbox['xnose'] = float(info[10])
            bbox['ynose'] = float(info[11])
            bbox['xleftmouth'] = float(info[12])
            bbox['yleftmouth'] = float(info[13])
            bbox['xrightmouth'] = float(info[14])
            bbox['yrightmouth'] = float(info[15])
